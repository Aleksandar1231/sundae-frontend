...
interface IZapper {
	/**
	 * Create LP (_to) from AVAX
	 */
	function zapAVAXToLP(address _to) external payable;

	/**
	 * Creates LP (_to) from a single asset (_from). Use this when the _from asset is not AVAX
	 */
...
// File contracts/interfaces/IJoeRouter02.sol
 ...
 * a STRAW/AVAX or FUDGE/DAI liquidity pair. And breaks a liquidity pair to single assets
 *
 */
contract Zapper is Ownable, IZapper, Pausable, ContractWhitelisted {
	...
	/**
	 * @dev Stores intermediate route information to convert a token to WAVAX
	 */
	...
	constructor(
		...
		IERC20(_FUDGE).safeApprove(address(ROUTER), type(uint256).max);
		IERC20(_STRAW).safeApprove(address(ROUTER), type(uint256).max);
		IERC20(_DAI).safeApprove(address(ROUTER), type(uint256).max);

		//approve to breakLP
		IERC20(_FUDGEDAI).safeApprove(address(ROUTER), type(uint256).max);
		IERC20(_STRAWAVAX).safeApprove(address(ROUTER), type(uint256).max);

		// set route pairs for our tokens
		// routePairAddresses[_STRAW] = _WAVAX;
		routePairAddresses[_FUDGE] = _DAI;

        //set default role as deployer
	...
	function zapAVAXToLP(address _to)
		external
		payable
		override
		whenNotPaused
	...
	function zapTokenToLP(
		...
		// STRAW, FUDGE, DAI to create STRAW-DAI or FUDGE-DAI will hit this if
		if ((_from == token0 || _from == token1) && token0 != WAVAX && token1 != WAVAX) {
	...
	function _swapAVAXToLP(
		...
		if (token0 != WAVAX)
			_swapAVAXForToken(token0, swapValue, address(this));
		
		if (token1 != WAVAX)
			_swapAVAXForToken(token1, amount.sub(swapValue), address(this));
		...
		if (token0 != WAVAX && token1 != WAVAX) {
			...
			address other = token0 == WAVAX ? token1 : token0;

			ROUTER.addLiquidityAVAX{value: token0 == WAVAX ? swapValue : amount.sub(swapValue)}(
				other, 
	 ...
	 * Not very useful for our current use case as bolt input currencies will only be DAI, WAVAX, FUDGE, STRAW
	...
	function _swap(
		...
		if (intermediate != address(0) && (_from == WAVAX || _to == WAVAX)) {
			// E.g. [WAVAX, DAI, STRAW/FUDGE] or [STRAW/FUDGE, DAI, WAVAX]
			path = new address[](3);
			path[0] = _from;
			path[1] = intermediate;
			path[2] = _to;
			...
			// E.g. [DAI, STRAW/FUDGE] or [STRAW/FUDGE, DAI]
			path = new address[](2);
			path[0] = _from;
			path[1] = _to;
		} else if (
			...
			// E.g. [FUDGE, DAI, STRAW] or [STRAW, DAI, FUDGE]
			path = new address[](3);
			path[0] = _from;
			path[1] = intermediate;
			path[2] = _to;
			...
			// [STRAW/STRAW, DAI, WAVAX, xRoute, xToken]
			path = new address[](5);
			path[0] = _from;
			...
			// E.g. [STRAW/FUDGE, DAI, WAVAX, xTokenWithWAVAXLiquidity]
			path = new address[](4);
			path[0] = _from;
			path[1] = intermediate;
			path[2] = WAVAX;
			...
			// E.g. [xTokenWithWAVAXLiquidity, WAVAX, DAI, STRAW/FUDGE]
			path = new address[](4);
			path[0] = _from;
			path[1] = WAVAX;
			path[2] = intermediate;
			...
			// E.g. [WAVAX, xTokenWithWAVAXLiquidity] or [xTokenWithWAVAXLiquidity, WAVAX]
			path = new address[](2);
			path[0] = _from;
			path[1] = _to;
		} else {
			// E.g. [xTokenWithWAVAXLiquidity, WAVAX, yTokenWithWAVAXLiquidity]
			path = new address[](3);
			path[0] = _from;
			path[1] = WAVAX;
			path[2] = _to;
	...
	/**
	 * Helps store intermediate route information to convert a token to WAVAX
	 */
